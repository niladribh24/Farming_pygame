...
class SoilLayer:
	...
	def _get_connected_drip_emitters(self):
		...
		# Add Tanks
		if hasattr(self, 'water_tank_sprites') and self.water_tank_sprites:
			for tank in self.water_tank_sprites.sprites():
				queue.append(tank)
				visited.add(tank)
				sources_found = True
		...
		while queue:
			...
			for drip in all_drips:
				if drip not in visited:
					if drip.rect.colliderect(search_rect):
						# Found a connected node
						visited.add(drip)
						connected_emitters.add(drip)
						queue.append(drip)
	...
	def auto_water_drip_tiles(self):
		...
		if disconnected_count > 0 and self.learning_system:
			self.learning_system.add_notification(f"⚠️ {disconnected_count} irrigation pipes not connected to Water Tank!")
